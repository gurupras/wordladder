...
//		}
		...
		tk.stop();
		Log.v(TAG, "Time taken to initialize WordLadder(" + difficulty + ") :" + tk.toString());
		activity.generatorInfoHandle(MainActivity.MessageHelper.getUpdateMessage(""));
	}
	...
	private List<String> findPath(String origin, String destination) {
		...
		while(!q.isEmpty()) {
			...
			for(int i = 0; i < wArray.length; i++) {
				char[] wCopy = new char[wArray.length];
				System.arraycopy(wArray, 0, wCopy, 0, wArray.length);
